...
class _PayoutSetupButton extends StatelessWidget {
  const _PayoutSetupButton({required this.user});
  final ProfileUser user;

  Future<void> _openBankForm(BuildContext context) async {
    await showModalBottomSheet(
      ...
      builder: (_) => _BankDetailsSheet(user: user),
    );
  }

  @override
  Widget build(BuildContext context) {
    final isSetup = user.hasBankAccount;
    return SizedBox(
      width: double.infinity,
      child: OutlinedButton.icon(
        icon: Icon(
...
class _BankDetailsSheet extends StatefulWidget {
  const _BankDetailsSheet({required this.user});
  final ProfileUser user;
...
class _BankDetailsSheetState extends State<_BankDetailsSheet> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // Refresh AppState so the button turns green immediately
      ...
      child: Form(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              'Earnings will be transferred to this account after each approved milestone.',
