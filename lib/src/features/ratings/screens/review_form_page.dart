...
/// Review hub: Write · Received · Given
///
...
/// - **Received** tab shows published reviews about the current user.
/// - **Given** tab shows reviews the current user has written, with Edit /
///   Delete actions (unless the review was removed by an admin).
...
class _ReviewCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final reviewerName = reviewer?.displayName ??
        (review.reviewerName.isNotEmpty ? review.reviewerName : 'User');
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            // Header row: stars + removed badge + date
            Row(
              children: [
                Row(
            ...
            if (isRemoved)
              Text(
                'This review was removed by our moderation team.',
                style: TextStyle(
                    color: cs.outline,
            ...
            // Action row (edit/delete for Given, nothing for Received)
            if (!isRemoved && mode == _CardMode.given) ...[
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(Icons.block_outlined, size: 11, color: Colors.red.shade700),
